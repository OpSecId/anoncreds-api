...
class TestWebvhRoutes:
    ...
    async def test_claim_types(self):
        ...
        assert len(cred_schema.get('claims')) == len(cred_schema.get('claim_indices')) == 4
        assert 'linkSecret' in cred_schema.get('blind_claims')
        
        claims = cred_schema.get('claims')
        assert next((claim for claim in claims if claim['claim_type'] == "Revocation"), None)
        assert next((claim for claim in claims if claim['claim_type'] == "Scalar"), None)
        assert next((claim for claim in claims if claim['claim_type'] == "Hashed"), None)
        assert next((claim for claim in claims if claim['claim_type'] == "Number"), None)
        
        assert next((claim for claim in claims if claim['label'] == "credentialId"), None)
